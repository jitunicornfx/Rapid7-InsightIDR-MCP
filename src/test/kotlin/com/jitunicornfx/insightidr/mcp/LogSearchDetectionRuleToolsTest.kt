...
class LogSearchDetectionRuleToolsTest {
    ...
    fun `detection rule crud maps to management-tags endpoints`() = runBlocking {
        ...
        assertEquals("/log_search/management/tags", h.lastRequest.url.encodedPath)

        h.call("logsearch_get_detection_rule", mapOf("rule_id" to "r1"))
        assertEquals("/log_search/management/tags/r1", h.lastRequest.url.encodedPath)
    ...
    fun `notification crud maps to management-actions endpoints`() = runBlocking {
        ...
        assertEquals("/log_search/management/actions", h.lastRequest.url.encodedPath)

        h.call("logsearch_get_notification", mapOf("notification_id" to "n1"))
        assertEquals("/log_search/management/actions/n1", h.lastRequest.url.encodedPath)
        ...
        assertEquals("/log_search/management/actions/n1/targets", h.lastRequest.url.encodedPath)
    ...
    fun `target crud maps to management-targets endpoints and builds the body`() = runBlocking {
        ...
        assertEquals("/log_search/management/targets", h.lastRequest.url.encodedPath)

        h.call("logsearch_get_target", mapOf("target_id" to "t1"))
        assertEquals("/log_search/management/targets/t1", h.lastRequest.url.encodedPath)
    ...
    fun `label crud maps to management-labels endpoints`() = runBlocking {
        ...
        assertEquals("/log_search/management/labels", h.lastRequest.url.encodedPath)

        h.call("logsearch_get_label", mapOf("label_id" to "lb1"))
        assertEquals("/log_search/management/labels/lb1", h.lastRequest.url.encodedPath)
