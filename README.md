...
```
build/libs/rapid7-insightidr-mcp-0.1.6-all.jar
```
...
```PowerShell
# PowerShell 5.1
powershell.exe -Command { $env:INSIGHTIDR_API_KEY="xxxxx"; $env:INSIGHTIDR_REGION="us"; java -jar .\rapid7-insightidr-mcp-0.1.6-all.jar --stdio }

# PowerShell 7
pwsh.exe -Command { $env:INSIGHTIDR_API_KEY="xxxxx"; $env:INSIGHTIDR_REGION="us"; java -jar .\rapid7-insightidr-mcp-0.1.6-all.jar --stdio }
...
INSIGHTIDR_API_KEY=xxxx INSIGHTIDR_REGION=us \
  java -jar build/libs/rapid7-insightidr-mcp-0.1.6-all.jar --stdio
```
...
# macOS / Linux
INSIGHTIDR_API_KEY=xxxx INSIGHTIDR_REGION=us \
  java -jar build/libs/rapid7-insightidr-mcp-0.1.6-all.jar --http --host 0.0.0.0 --port 3001
```
...
```json
{
  "mcpServers": {
    "insightidr": {
      ...
      "args": [
        "-jar",
        "C:\\MCP Dev\\Rapid7-InsightIDR-MCP\\build\\libs\\rapid7-insightidr-mcp-0.1.6-all.jar",
        "--stdio"
      ],
      "env": {
